...
static JslCValueType expression_type(const JslAstNode *node, const JslLocal *locals) {
    switch (node->kind) {
        ...
        case JSL_AST_LITERAL_EXPRESSION: return node->as.literal_expression.kind == JSL_AST_LITERAL_INTEGER ? JSL_C_VALUE_I32 : node->as.literal_expression.kind == JSL_AST_LITERAL_STRING ? JSL_C_VALUE_STRING : JSL_C_VALUE_INVALID;
        case JSL_AST_UNARY_EXPRESSION: case JSL_AST_BINARY_EXPRESSION: return JSL_C_VALUE_I32;
...
static int emit_expression(const JslAstNode *node, const JslLocal *locals, FILE *output, JslCBackend *backend) {
    switch (node->kind) {
        case JSL_AST_IDENTIFIER_EXPRESSION:
            if (local_type(locals, node->as.identifier_expression.name) == JSL_C_VALUE_INVALID) { set_error(backend, node->position, "unknown local in v0.0.7 backend"); return 0; }
        ...
        case JSL_AST_LITERAL_EXPRESSION:
            ...
            set_error(backend, node->position, "v0.0.7 backend supports integer and string literals only"); return 0;
        ...
        case JSL_AST_CALL_EXPRESSION:
            ...
            if (node->as.call_expression.callee->kind != JSL_AST_IDENTIFIER_EXPRESSION) { set_error(backend, node->position, "v0.0.7 backend supports direct function calls only"); return 0; }
...
static int emit_console_statement(const JslAstNode *call, const JslLocal *locals, FILE *output, JslCBackend *backend) {
    ...
    if (type == JSL_C_VALUE_INVALID) { set_error(backend, argument->position, "console output supports i32 and string values only"); return -1; }
...
static int emit_parameters(const JslAstNode *function, FILE *output, JslCBackend *backend) {
    for (size_t i = 0; i < function->as.function_declaration.parameter_count; i++) {
        ...
        if (!text_equals(parameter->type_name, "i32")) { set_error(backend, parameter->position, "v0.0.7 backend supports i32 parameters only"); return 0; }
...
static int emit_function_signature(const JslAstNode *function, FILE *output, JslCBackend *backend, int prototype) {
    if (!text_equals(function->as.function_declaration.return_type, "i32")) { set_error(backend, function->position, "v0.0.7 backend supports i32 return types only"); return 0; }
...
static int emit_function_body(const JslAstNode *function, FILE *output, JslCBackend *backend) {
    ...
    for (size_t i = 0; success && i < statements->count; i++) {
        ...
        if (statement->kind == JSL_AST_VARIABLE_STATEMENT) {
            ...
            if (result == 0) { set_error(backend, statement->position, "v0.0.7 backend supports console output calls only as expression statements"); success = 0; }
            else success = result > 0;
        } else { set_error(backend, statement->position, "unsupported statement in v0.0.7 backend"); success = 0; }
    }
